...
/** Soften ROM audio a bit so it sits closer to shell SFX (UI still shows 0–100%). */
const GAME_VOLUME_SCALE = 0.4;

function volumeToGameGain(volume) {
  const v = typeof volume === 'number' ? volume : 0;
  if (!v || v <= 0) return 0;
  return Math.max(0, Math.min(1, (v / 100) * GAME_VOLUME_SCALE));
}
...
class EmulatorManager {
  ...
  init() {
    ...
    setTimeout(() => {
      ...
      if (window.settings) {
        window.settings[3] = volumeToGameGain(initialVolume);
      }
  ...
  setVolume(volume) {
    ...
    const volumeDecimal = volumeToGameGain(volume);
  ...
  loadStateFromFile(file) {
    ...
    reader.onload = (e) => {
      try {
        ...
        alert(
          'Error loading state: ' +
            (error instanceof SyntaxError ? 'File is corrupted' : error.message),
        );
      }
    };
