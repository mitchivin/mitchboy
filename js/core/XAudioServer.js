...
XAudioServer.prototype.initializeMediaStream = function () {
  ...
  XAudioJSMediaStreamWorker = new Worker(
    XAudioJSsourceHandle.substring(0, XAudioJSsourceHandle.length - 3) + 'MediaStreamWorker.js',
  );
...
function XAudioJSGenerateFlashSurroundString() {
  ...
  for (
    ...
    XAudioJSBinaryString[XAudioJSTotalSamples++] = String.fromCharCode(
      ((Math.min(Math.max(XAudioJSResampledBuffer[XAudioJSResampleBufferStart++] + 1, 0), 2) *
        0x3fff) |
        0) +
        0x3000,
    );
...
function XAudioJSGenerateFlashStereoString() {
  ...
  for (
    var index = 0;
    index < XAudioJSTotalSamples && XAudioJSResampleBufferStart != XAudioJSResampleBufferEnd;
